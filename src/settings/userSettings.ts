...
function onSaveTimeout(this: any) {
  const self = this;
...
function saveServerPreferences(instance: any) {
...
export class UserSettings {
   ...
   */
  setUserInfo(userId: string, apiClient: any) {
    if (this.saveTimeout) {
   ...
   */
  set(name: string, value: any, enableOnServer?: boolean) {
   ...
   */
  get(name: string, enableOnServer?: boolean) {
    const userId = this.currentUserId;
    if (enableOnServer !== false && this.displayPrefs) {
   ...
   */
  serverConfig(config: any) {
    const apiClient = this.currentApiClient;
    if (config) {
      return apiClient.updateUserConfiguration(this.currentUserId, config);
    }

    return apiClient.getUser(this.currentUserId).then(function (user: any) {
  ...
  preferFmp4HlsContainer(val?: boolean) {
    ...
    return toBoolean(
      ...
      browser.safari ||
        browser.firefox ||
        (browser.chrome && !browser.web0s && !browser.tizen) ||
   ...
   */
  useEpisodeImagesInNextUpAndResume(val?: string | boolean) {
    if (val !== undefined) {
      return this.set(
   ...
   */
  backdropScreensaverInterval(val?: number) {
    if (val !== undefined) {
   ...
   */
  enableRewatchingInNextUp(val?: boolean) {
    if (val !== undefined) {
   ...
   * @return {Query} Query.
   */
  loadQuerySettings(key: string, query: any) {
   ...
   */
  saveQuerySettings(key: string, query: any) {
    const values: any = {};
    if (query.SortBy) {
   ...
   */
  saveViewSetting(key: string, value: string) {
   ...
   * @param {string|undefined} key - Settings key.
   */
  setSubtitleAppearanceSettings(value: Object, key?: string) {
    key = key || 'localplayersubtitleappearance3';
   ...
   */
  getComicsPlayerSettings(mediaSourceId: string) {
   ...
   * @param {string} mediaSourceId - Media Source Id.
   */
  setComicsPlayerSettings(value: Object, mediaSourceId: string) {
    const settings = JSON.parse(
      this.get('comicsPlayerSettings', false) || '{}',
    );
